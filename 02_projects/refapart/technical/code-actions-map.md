...
| `REFAPART.QUOTE.CREATE_REQUEST` | Publicar solicitud de pieza | form | REFAPART | Cotizacion | `/publicar-busqueda` | `app/publicar-busqueda/page.tsx` | PENDIENTE_DE_DEFINIR | `/projects/REFAPART/requests` | `API.PY.DJANGO.RefaPart` | `marketplace.views.requests_create` | POST | Publico / opcional usuario | `RefaPartPartRequest`, `RefaPartAuditLog` | loading / error / success | validation error, Gateway unavailable | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `api-contracts.md` | vigente | 2026-06-27 |
| `REFAPART.QUOTE.SEARCH_PART` | Buscar refaccion | form | REFAPART | Busqueda | `/`, `/resultados` | `app/page.tsx`, `app/resultados/page.tsx` | PENDIENTE_DE_DEFINIR | `/projects/REFAPART/search` | `API.PY.DJANGO.RefaPart` | `marketplace.views.search` | POST | Publico | `RefaPartSearchQueryLog`, `RefaPartPartRequest` si no hay resultado | loading / empty / error / success | search validation error, Gateway unavailable | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `api-contracts.md` | vigente | 2026-06-27 |
| `REFAPART.QUOTE.SEND_TO_SUPPLIERS` | Enviar solicitud a proveedores internos | endpoint | REFAPART | Operacion interna | `/admin/refapart/part-requests/[id]/suppliers` | `app/admin/refapart/part-requests/[id]/suppliers/page.tsx` | PENDIENTE_DE_DEFINIR | `/projects/REFAPART/admin/part-requests/{id}/supplier-message` | `API.PY.DJANGO.RefaPart` | `marketplace.views.supplier_message` | POST | PENDIENTE_DE_DEFINIR: rol operativo REFAPART | `RefaPartSupplierMessage`, `RefaPartAuditLog` | loading / error / success | supplier missing, request not found | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `api-contracts.md` | vigente | 2026-06-27 |
| `REFAPART.SUPPLIER.INTERNAL_RESPONSE` | Registrar oferta de proveedor | form | REFAPART | Proveedores internos | `/admin/refapart/part-requests/[id]/quote` | `app/admin/refapart/part-requests/[id]/quote/page.tsx` | PENDIENTE_DE_DEFINIR | `/projects/REFAPART/admin/part-requests/{id}/offers` | `API.PY.DJANGO.RefaPart` | `marketplace.views.offers` | POST | PENDIENTE_DE_DEFINIR: rol operativo REFAPART | `RefaPartSupplierPartOffer`, `RefaPartPartRequest`, `RefaPartAuditLog` | loading / error / success | supplier invalid, request not found, validation error | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `api-contracts.md` | vigente | 2026-06-27 |
| `REFAPART.ADDRESS.SEARCH` | Buscar direccion | service | REFAPART | Address | `/cuenta/direcciones`, `/checkout` | `app/cuenta/direcciones/page.tsx`, `app/checkout/page.tsx` | PENDIENTE_DE_DEFINIR | PENDIENTE_DE_DEFINIR: ruta Gateway Address | `API.PY.DJANGO.Address` | PENDIENTE_DE_DEFINIR | GET | Usuario autenticado en rutas cuenta; opcional en checkout | Address API; snapshots locales permitidos | loading / empty / error / success | postal code not found, Gateway unavailable | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `integrations/00_address_api_usage.md` | pendiente | 2026-06-27 |
| `REFAPART.PAYMENT.REQUEST` | Solicitar pago | integration | REFAPART | Payments | `/checkout` | `app/checkout/page.tsx` | PENDIENTE_DE_DEFINIR | PENDIENTE_DE_DEFINIR | Payments API futura | PENDIENTE_DE_DISEÑO_PRIORITARIO | POST | Usuario autenticado o checkout controlado | `RefaPartOrder`, futuro Payment | loading / error / success | provider unavailable, payment rejected | NO_EJECUTADO_POR_INSTRUCCION_DEL_USUARIO | `business/02_payment_policy.md` | pendiente | 2026-06-27 |

## Pendientes Refapart derivados

- Auth: datos reales de prueba y matriz final de permisos por rol.
- Gateway: confirmar rutas Address exactas antes de marcar `REFAPART.ADDRESS.SEARCH` como vigente.
...
- Code actions: completar servicios frontend concretos de busqueda, solicitud, checkout y admin si se separan del codigo de pagina.

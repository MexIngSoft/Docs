...
| `REFAPART.ADDRESS.SEARCH` | Buscar y crear direccion | service | REFAPART | Address | `/cuenta/direcciones`, `/checkout` | `app/cuenta/direcciones/page.tsx`, `app/checkout/page.tsx`, `components/AddressForm.tsx` | `lib/api/address.ts -> suggest/create` | `/core/address/api/address/suggest?postalCode=`, `/core/address/api/address/addresses` | `API.PY.DJANGO.Address` | `/api/address/suggest?postalCode=`, `/api/address/addresses` | GET/POST | Usuario autenticado en rutas cuenta; opcional en checkout | Address API; `RefaPartOrder.ShippingAddressId` | loading / empty / error / success | postal code not found, Gateway unavailable, missing shippingAddressId | Web lint/build; API check; Address check | `integrations/00_address_api_usage.md` | vigente | 2026-06-29 |

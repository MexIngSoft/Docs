...
| `REFAPART.AUTH.LOGOUT` | `/logout` | `app/logout/page.tsx` | `features/auth/services/auth-service.ts -> logout` | `/auth/logout/` | vigente | Limpia sesion via Gateway y redirige a `/resultados`; el header debe navegar primero a `/logout` para evitar que `AuthGuard` de una ruta protegida mande a `/login`. |
| `REFAPART.AUTH.RETURN_TO` | `/login`, `/register`, `/logout` | `features/auth/return-to.ts` | `sanitizeReturnTo`, `resolveReturnTo`, `resolveLogoutPath` | `N/A` | vigente | Conserva continuidad comercial sin aceptar redirects externos ni rutas Auth como destino. |
...
| `REFAPART.ADMIN.PRODUCTS` | `/admin/refapart/products` | `app/admin/refapart/products/page.tsx`, `ProductAdminPanel.tsx` | `lib/api/refapart.ts -> productsAdmin/createProduct/updateProduct/deleteProduct` | `/projects/REFAPART/admin/products`, `/projects/REFAPART/admin/products/{slug}` | vigente | Alta, edicion, publicacion/ocultamiento y eliminacion de productos REFAPART. |
...
| `REFAPART.ACCOUNT.DASHBOARD` | `/dashboard`; `/cuenta` alias | `app/dashboard/page.tsx`, `app/cuenta/page.tsx`, `features/account/components/CustomerDashboard.tsx` | `useAuth`; futuro `GET /api/v1/projects/REFAPART/customer/dashboard` | `GET /api/v1/projects/REFAPART/customer/dashboard` | vigente_parcial | `/dashboard` es el dashboard cliente; `/cuenta` redirige como alias para evitar pantalla intermedia; incluye acciones secundarias de perfil, configuracion, direcciones, busqueda y solicitud; `MOCK_TEMPORAL` hasta cerrar endpoint real. |
